...
from apps_generator.utils.naming import title_case, pascal_case, snake_case, camel_case
from apps_generator.cli.resources import (
    parse_resources as _parse_resources,
    generate_resource_scaffolding as _generate_resource_scaffolding,
    generate_resource_types as _generate_resource_types,
    find_java_root as _find_java_root,
    find_resources_root as _find_resources_root,
    find_api_client_src as _find_api_client_src,
)
...
def generate(
    ...
    if template_info.name == "api-domain" and not dry_run:
        ...
        resources = _parse_resources(resources_str)

        if resources:
            java_root = _find_java_root(result, project_name, base_package)
            res_root = _find_resources_root(result, project_name)
            if java_root and res_root:
                _generate_resource_scaffolding(java_root, res_root, resources, base_package, project_name)

            # Generate TypeScript types in api-client if linked
            if api_client is not None:
                api_client_src = _find_api_client_src(api_client)
                if api_client_src:
                    _generate_resource_types(api_client_src, resources)
    ...
    for page in pages:
        ...
        if not page_file.exists():
            if resource and page_type == "list":
                _write_list_page(page_file, component_name, label, resource, fields)
            elif resource and page_type == "form":
                _write_form_page(page_file, component_name, label, resource, fields)
            else:
